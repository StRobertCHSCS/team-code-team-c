...
void loop() {
  ...
  prizm.setServoPosition(1,180);        // Move servo #1 to 180 degree position. First number (#2) is ID of expansion controller.
  delay(3000);                          // wait here for 3 seconds while servo moves to position
  prizm.setServoPosition(1,0);          // Move servo #1 to 0 degree position. First number (#2) is ID of expansion controller.
